...
def main():
    ...
    parser.add_argument("--no-overwrite-audio", action="store_true", help="reserve part of audios that have been generated")
    ...
    if args.debug:
        ...
        if args.no_overwrite_audio:
            skip_num = 0
            for i in tqdm(ids_data):
                fname_out_name = dataset[i][3]
                output_file = os.path.join(args.output_dir, fname_out_name + "_gen.wav")
            ...
            for i in tqdm(ids_data):
                inference(i)
        if skip_num > 0:
            print(f"Skip {skip_num} audios that have existed in output folder.")
