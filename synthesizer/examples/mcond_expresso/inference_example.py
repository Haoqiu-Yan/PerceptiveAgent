...
from examples.mcond_expresso.dataset_example import InferenceMultiConditionsCodeDataset
...
def load_generator(checkpoint_file, cuda_idx):
    generator = MultiConditionsCodeGenerator(h).to(cuda_idx)
...
def load_vocoder_meta(vocoder_ckpt):
    ...
    # ---add by haoqiu---
    if os.path.isdir(vocoder_ckpt):
        ckpt_dir = vocoder_ckpt
    else:
    ...
    if os.path.exists(spkr_file):
        ...
        print(f"Loaded {len(speakers)} speakers. First few speakers: {speakers[:4]}")
    ...
    if os.path.exists(style_file):
        ...
        print(f"Loaded {len(styles)} styles. First few styles: {styles[:4]}")

    pitch_file = os.path.join(ckpt_dir, "pitchs.txt")
    pitchs = None
    if os.path.exists(pitch_file):
        with open(pitch_file) as f:
            pitchs = [line.strip() for line in f]
        print(f"Loaded {len(pitchs)} pitchs. First few pitchs: {pitchs[:4]}")
    
    speed_file = os.path.join(ckpt_dir, "speeds.txt")
    speeds = None
    if os.path.exists(speed_file):
        with open(speed_file) as f:
            speeds = [line.strip() for line in f]
        print(f"Loaded {len(speeds)} speeds. First few speeds: {speeds[:4]}")
    
    energy_file = os.path.join(ckpt_dir, "energys.txt")
    energys = None
    if os.path.exists(energy_file):
        with open(energy_file) as f:
            energys = [line.strip() for line in f]
        print(f"Loaded {len(energys)} energys. First few energys: {energys[:4]}")

    return speakers, styles, pitchs, speeds, energys
...
def main():
    ...
    speakers, styles, pitchs, speeds, energys = load_vocoder_meta(args.checkpoint_file)
    dataset = InferenceMultiConditionsCodeDataset(
