"""
LOCATION: /data1/haoqiuyan/ws_iwslt/code/parse_hubert_codes_zh.py (有修改)
为训练vocoder准备数据集,
把units和manifest文件组合在一起, 
新生成的文件形式为
{'audio': path, 'hubert': XX, 'duration': xx, 'spk': xx, 'style': xx,}
...
python code/synthesis/hubert/gen_datasets_style-only.py --codes /speech_synthesis/hubert_kmn_getcodes/units/ljspeech/transcript.units --manifest ./speech_synthesis/hubert_kmn_getcodes/manifest/ljspeech/manifest_all.tsv --outdir /data1/haoqiuyan/ws_iwslt/output/vocoder_in --min-dur 0.01 --tt 0.05 --cv 0.05 --spk-prefix lj
#  --spk-from-filename is set as False
"""
...
def split(args, samples):
    """
    读取train/test/val都有哪些音频
    Args: args.ref_train -- 提供train.manifest
          args.tt --提供划分比例
    """
