"""
get response from ChatGPT, conversation generation
---
Usage

python code/openai/conversation_chat.py --id-convs ./framework/meld/id_convs.txt --audio-asr-caption ./framework/meld/audio_asr_caption.txt --save-root ./framework/openai/test 
...
def process_gpt_response(response):
    # re.S: force `.` to match all characters including `\n`
    matchObj = re.match(r'(.*)\n(.*)', response, re.S)
    asr = matchObj.group(1)
    caption = matchObj.group(2)
...
if __name__ == '__main__':
    ...
    for sample in sample_iterator:
        ...
        gt_outputL.append("{}\t{}\t{}".format(sample_count, cur_asr, cur_caption))

        try:
            gen_asr, gen_caption = process_gpt_response(chat_content)
            gen_outputL.append("{}\t{}\t{}".format(sample_count, gen_asr, gen_caption))
