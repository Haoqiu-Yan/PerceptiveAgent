"""改了一部分代码然后废掉了, 基于app.py, 一个网页显示的脚本"""
import argparse
import os
import random
...
from eval_scripts.infer_caption import InferCaption
...
def parse_args():
    ...
    parser.add_argument("--cfg-path", help="path to configuration file.", default='./eval_configs/mmgpt4_eval.yaml', required=True)
    parser.add_argument("--output-dir", help="directory to save predicted & real files.", required=True)
...
if __name__ == '__main__':
    ...
    infer_module = InferCaption(cfg)
    trueList, preList = infer_module.model()
    save_infer_to(args.output_dir)
