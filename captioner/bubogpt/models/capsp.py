...
@registry.register_model("capsp")
class CAPSP(BaseModel):
    """
    ...
    def __init__(
        ...
        self.sp_processor = AutoProcessor.from_pretrained("facebook/wav2vec2-base-960h")
        self.sp_encoder = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-base-960h")
        if freeze_wav2vec:
            for name, param in self.sp_encoder.named_parameters():
                param.requires_grad = False
            self.sp_encoder = self.sp_encoder.eval()
            self.sp_encoder.train = disabled_train
    ...
    def encode_inputs(self, sp_input: Tensor) -> Tensor:
        wav2vec_out = {}
        with self.maybe_autocast():
            # imagebind_outputs = self.multimodal_encoder(inputs)
            # llama_inputs = self.multimodal_joiner(imagebind_outputs)
    ...
    def forward(self, inputs: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        loss = outputs.loss
        # add by haoqiu
        # logging.info("Loss Device---\nbos_embeds:{}, input_embs:{}, to_regress_embeds:{}\natts_bos:{}, input_atts:{}, to_regress_tokens.attention_mask:{}".format(bos_embeds.device, input_embs.device, to_regress_embeds.device, atts_bos.device, input_atts.device, to_regress_tokens.attention_mask.device))
        return {"loss": loss}
